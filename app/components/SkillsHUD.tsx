...
export default function SkillsHUD() {
  return (
    <section className="h-screen flex items-center justify-center py-16 sm:py-20 px-4 sm:px-6 relative z-10 border-t border-purple-500/20 snap-start overflow-hidden">
      {/* LLaMA Architecture Background - Desktop Only */}
      <div className="hidden xl:block absolute inset-0 overflow-hidden opacity-40 pointer-events-none">
      ...
      <div className="max-w-6xl mx-auto w-full h-full flex flex-col py-4">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-6 sm:mb-8 md:mb-12 font-['Orbitron'] text-center flex-shrink-0">
        ...
        {/* Mobile List View */}
        <div className="md:hidden flex-1 overflow-y-auto overflow-x-hidden px-2">
          <div className="space-y-3">
            {skills.map((skill, index) => (
              <div
                key={index}
                className="p-4 border border-green-500/30 bg-black/70 holographic rounded"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                    <span className="text-green-400 font-bold font-['Orbitron'] text-base">
                      {skill.name}
                    </span>
                  </div>
                  <span className="text-cyan-400 text-xl font-bold font-['Orbitron']">{skill.level}%</span>
                </div>
                ...
                <div className="text-xs text-gray-400 font-['Space_Mono']">
                  [{skill.category}]
                </div>
              </div>
            ))}
        ...
        <div className="hidden md:block relative flex-1 overflow-y-auto overflow-x-hidden pr-2">
          {/* Scrollbar styling */}
          <style jsx>{`
          ...
          {/* Center radar effect - Desktop Only */}
          <div className="hidden lg:flex absolute inset-0 items-center justify-center opacity-10 pointer-events-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6 relative z-10 pb-4">
            {skills.map((skill, index) => (
              <div
                key={index}
                className="group relative p-4 sm:p-5 md:p-6 border border-green-500/30 bg-black/70 hover:border-cyan-400 hover:bg-black/85 transition-all duration-300 holographic holo-border iridescent"
                style={{
              ...
              >
                ...
                {/* Skill Name */}
                <div className="text-green-400 font-bold font-['Orbitron'] mb-4 sm:mb-6 text-base sm:text-lg group-hover:animate-[glowIntensify_0.5s_ease-in-out,colorShift_2s_ease-in-out_infinite]">
                  {skill.name}
                </div>
                ...
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-cyan-400 text-xl sm:text-2xl font-bold font-['Orbitron']">{skill.level}</span>
                    <span className="text-green-500 text-[10px] sm:text-xs font-['Space_Mono']">LEVEL</span>
                  </div>
                </div>
                ...
                <div className="mb-3 sm:mb-4">
                  <div className="flex items-center justify-between mb-1 sm:mb-2">
                    <span className="text-[10px] sm:text-xs text-gray-400 font-['Space_Mono']">PROFICIENCY</span>
                    <span className="text-[10px] sm:text-xs text-cyan-400 font-['Space_Mono'] font-bold">{skill.level}%</span>
                  </div>
                  <div className="h-1.5 sm:h-2 bg-gray-800 rounded-full overflow-hidden relative">
                    <div 
                      className="absolute inset-0 bg-gradient-to-r from-green-500 via-cyan-400 to-purple-500 transition-all duration-1000 ease-out rounded-full"
                      style={{ width: `${skill.level}%` }}
                    >
                ...
                {/* Mini Stats */}
                <div className="grid grid-cols-3 gap-1.5 sm:gap-2 text-center text-[10px] sm:text-xs font-['Space_Mono']">
                  <div className="bg-black/50 p-1.5 sm:p-2 border border-green-500/20 rounded">
                    <div className="text-green-400 font-bold text-xs sm:text-sm">{Math.floor(skill.level * 0.95)}</div>
                    <div className="text-gray-500 text-[8px] sm:text-[10px]">ACC</div>
                  </div>
                  <div className="bg-black/50 p-1.5 sm:p-2 border border-cyan-400/20 rounded">
                    <div className="text-cyan-400 font-bold text-xs sm:text-sm">{Math.floor(skill.level * 0.98)}</div>
                    <div className="text-gray-500 text-[8px] sm:text-[10px]">SPD</div>
                  </div>
                  <div className="bg-black/50 p-1.5 sm:p-2 border border-purple-400/20 rounded">
                    <div className="text-purple-400 font-bold text-xs sm:text-sm">{Math.floor(skill.level / 25)}Y</div>
                    <div className="text-gray-500 text-[8px] sm:text-[10px]">EXP</div>
                  </div>
                </div>
              </div>
            ))}
